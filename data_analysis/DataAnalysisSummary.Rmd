...
rmr.obs <-  resp3 %>% 
  mutate(Pred=predict(rmr.3.p2a, re.form=NA, type='response'),
...
mmr.obs <-  resp4 %>% 
  mutate(Pred=predict(mmr.1.p2a, re.form=NA, type='response'),
...
nas.obs <-  resp4 %>% 
  mutate(Pred=predict(nas.1.p2a, re.form=NA, type='response'),
...
pha.2.p3a  %>% performance::check_model(detrend = FALSE)
```
...
pha.2.p3a.log %>% performance::check_model(detrend = FALSE)
```

##### Gaussian (inverse)
```{r immuno-model-valid-2.2c, fig.width=8, fig.height=8, message=FALSE, warning=FALSE, echo=FALSE}
pha.2.p3a.inv %>% performance::check_model(detrend = FALSE)
```
...
aic.c=AICc(pha.2.p3a, pha.2.p3a.gamma, k=2)
bic=BIC(pha.2.p3a, pha.2.p3a.gamma)

model.fit.table <- as.data.frame(aic.c) %>% 
  tibble::rownames_to_column("model") %>%
  mutate(BIC = bic$BIC, 
         r2m = c(r.squaredGLMM(pha.2.p3a)[1], r.squaredGLMM(pha.2.p3a.gamma)[1]), 
         r2c = c(r.squaredGLMM(pha.2.p3a)[1], r.squaredGLMM(pha.2.p3a.gamma)[1]))
model.fit.table %>% knitr::kable(format = "html")  %>%
  kable_paper(full_width = TRUE)
```
...
#pha.2.p3a.gamma %>% performance::check_model(detrend = FALSE)
```
...
pha.2.p3a.gamma %>% ggemmeans(~TEMPERATURE|REGION) %>% plot(add.data=TRUE, jitter=c(0.05,0))
```

#### plot_model 

```{r immuno-partial-plots-2, echo=FALSE, fig.width=8, fig.height=6}
pha.2.p3a.gamma %>% plot_model(type='std', vline.color = "grey12", show.values=TRUE, sort.est = TRUE, transform = NULL, value.offset = 0.3)
```
...
as.data.frame(summary(pha.2.p3a.gamma)$coefficients[1]) %>% 
  dplyr::rename( 
    Estimate = cond.Estimate,
    StdError = cond.Std..Error,
    Zvalue = cond.z.value,
...
pha.2.p3a.gamma %>% performance::r2_nakagawa(tolerance=1e-09) %>% 
  as.data.frame() %>%
  knitr::kable(format = "html")  %>%
  kable_paper(full_width = TRUE)
```
...
pha.obs <-  pha2 %>% 
  mutate(Pred=predict(pha.2.p3a.gamma, re.form=NA),
...
caption_text <- "<br> Supplemental figure 4: Comparison of hematocrit ratios, that were measured at 31.5°C, between low- (red) and high-latitudinal (blue) populations. No significant difference was observed between the different latitudes (p =0.058). Solid (low-latitude) and dashed (high-latitude) lines represent 95% confidence intervals."

hema.plot <- hema.plot + labs(caption = caption_text) + 
  theme(plot.caption=element_textbox_simple(padding = margin(0,10,5,0))); hema.plot
...
caption_text <- "<br> Supplemental figure 3: Density plots displayed fish body size to chamber ratios. Fish that were sampled for aerobic physiology from the low-latitude region are represented in red; fish from the high-latitude region are represent in blue."

mass.distr <- mass.distr + labs(caption = caption_text) + 
