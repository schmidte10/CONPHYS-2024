...
rest_MgO2.hr <- glmmTMB(RESTING_MgO2.hr ~ 1+ REGION * TEMPERATURE + RESTING_CHAMBER + MASS_CENTERED + RESTING_SUMP + (1|REGION:POPULATION) + (1|FISH_ID), 
                family=gaussian(),
                data = resp3,
...
rest.poly2_MgO2.hr <- glmmTMB(RESTING_MgO2.hr ~ 1+ REGION * poly(TEMPERATURE, 2) + MASS_CENTERED + RESTING_CHAMBER + RESTING_SUMP + (1|REGION:POPULATION) + (1|FISH_ID), 
                      family=gaussian(),
                      data = resp3,
                      control=glmmTMBControl(optimizer=optim,
                                             optArgs = list(method='BFGS')),
...
rest.poly3_MgO2.hr <- glmmTMB(RESTING_MgO2.hr ~ 1+ REGION * poly(TEMPERATURE, 3) + MASS_CENTERED + RESTING_CHAMBER + RESTING_SUMP + (1|REGION:POPULATION) + (1|FISH_ID), 
                      family=gaussian(),
                      data = resp3,
                      control=glmmTMBControl(optimizer=optim,
                                             optArgs = list(method='BFGS')),
...
AICc(rest_MgO2.hr, rest.poly2_MgO2.hr , rest.poly3_MgO2.hr, k = 2, REML = TRUE)
check_model(rest_MgO2.hr) 

#--- save model ---#
saveRDS(rest_MgO2.hr, file = "glmmTMB_rest_MgO2.hr.RDS") 

#--- load model ---#
#rest.poly3 <- readRDS("glmmTMB_restpoly3.RDS")

#--- investigate model ---#
rest_MgO2.hr %>% plot_model(type='eff',  terms=c('TEMPERATURE','REGION'), show.data=TRUE)
rest_MgO2.hr %>% ggemmeans(~TEMPERATURE|REGION) %>% plot(add.data=T, jitter=c(0.05,0))
rest_MgO2.hr %>% plot_model(type='est')

rest_MgO2.hr %>% summary()
rest_MgO2.hr %>% confint()
rest_MgO2.hr  %>% r.squaredGLMM()
rest_MgO2.hr  %>% performance::r2_nakagawa()

rest_MgO2.hr %>% emtrends("REGION", var="TEMPERATURE") %>% pairs() %>% summary(infer=TRUE)
#--- summary ---# 
obs %>% 
  group_by(REGION, TEMPERATURE)  %>%    
  dplyr::summarise(sample_size = n(), 
                   ...
                   Mean = mean(Pred)) 
#--- plot ---#
newdata <- rest_MgO2.hr %>% ggemmeans(~TEMPERATURE|REGION) %>%
  as.data.frame %>% 
...
predict(rest_MgO2.hr, re.form=NA) 
#data points based on month and situation - to get the group means
residuals(rest_MgO2.hr, type='response') 
#data points based on month/situation/random effects - to get the data points
obs <-  resp3 %>% 
  mutate(Pred=predict(rest_MgO2.hr, re.form=NA),
         Resid = residuals(rest_MgO2.hr, type='response'),
         Fit = Pred + Resid)
obs %>% head() 
g2 <- ggplot(newdata, aes(y=predicted, x=TEMPERATURE, color=group, fill = group)) + 
  #geom_point(data = obs, mapping = aes(y=Fit, x=TEMPERATURE, color = REGION, fill = REGION),
             # alpha = 0.20,
  ...
  theme_classic() + ylab("RESTING_MgO2.hr (RMR)")+
...
#--- save model ---# 
saveRDS(pop.rest.poly2_MgO2.hr, file = "glmmTMB_rest_p2_population_MgO2_hr.RDS") 
